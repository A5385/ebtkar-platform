...
const GridWrapper = ({ children, cols = 1, className, title }: GridWrapperProps) => {
    return (
        <div
            className={cn(
                'grid gap-4',
                title && 'relative mt-2 rounded-xl border p-6',
                GridMapper[cols],
                className,
            )}
        >
            {title && (
                <p className={cn('bg-card inset-s-5 absolute -top-5 px-2  rounded-xl')}>{title}</p>
            )}
            {children}
        </div>
    );
