...
export class UserController {
    ...
    @MessagePattern(MSM_PATTERN['auth-service'].user.register)
    register(@Payload() dto: RegisterDto) {
        return this.userService.register(dto);
    }

    @MessagePattern(MSM_PATTERN['auth-service'].user.getAll)
    async getAllUsers() {
        return await this.userService.getAllUsers();
    }

    @MessagePattern(MSM_PATTERN['auth-service'].user.findById)
    async findUserById(@Payload() dto: { userId: string }) {
        return await this.userService.findUserById(dto.userId);
    }

    @MessagePattern(MSM_PATTERN['auth-service'].user.findByEmail)
    async findUserByEmail(@Payload() dto: { email: string }) {
        return await this.userService.findUserByEmail(dto.email);
    }
    @MessagePattern(MSM_PATTERN['auth-service'].user.changePassword)
